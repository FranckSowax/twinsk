...
import { byListing, byTransport, effectiveCurrency, monthlySeries, orderMargin, totalsInFcfa, type BusinessOrder } from './admin-business';
...
const base: BusinessOrder = {
  ...
  items_total: 122_941, // 1351 CNY × 91 — cohérent avec les lignes
...
describe('marge d’une commande payée', () => {
  it('retrouve le prix d’achat depuis la marge et isole le transport', () => {
    ...
    expect(m.revenue).toBeCloseTo(1351 * 91, 0);
    expect(m.margin).toBeCloseTo((1351 - achatCny) * 91, 0);
    expect(m.marginRate).toBeGreaterThan(0.25);
    expect(m.transport).toBe(1_073_460); // jamais compté dans la marge
    expect(m.collected).toBeCloseTo(1351 * 91 + 1_073_460, 0);
  });
  it('la remise réduit le chiffre d’affaires et la marge, pas le coût', () => {
    const m = orderMargin({ ...base, discount: 20_000 });
    expect(m.revenue).toBeCloseTo(1351 * 91 - 20_000, 0);
    expect(m.margin).toBeCloseTo(orderMargin(base).margin - 20_000, 0);
  });
  ...
  it('marge inconnue (0 %) : prix d’achat = prix affiché, marge nulle sur la ligne', () => {
    const m = orderMargin({ ...base, items_total: 91_000, transport_cost: 0, lines: [{ product_id: 'p1', quantity: 10, unit_price_cny: 100, margin_percent: 0 }] });
...
describe('cumuls et séries', () => {
  const eur: BusinessOrder = { ...base, id: 'o2', currency: 'EUR', items_total: 100, transport_cost: 50, lines: [{ product_id: 'p1', quantity: 1, unit_price_cny: 770, margin_percent: 100 }] };
  it('convertit toutes les devises en FCFA pour le cumul', () => {
    const t = totalsInFcfa([base, eur]);
    expect(t.orders).toBe(2);
    // 770 CNY sur un listing en euros = 100 € → 100 × 7,7 × 91 FCFA
    expect(t.revenue).toBeCloseTo(1351 * 91 + 100 * 7.7 * 91, 0);
...
describe('devise du listing changée après la commande', () => {
  // Commande passée en FCFA, listing basculé en euros depuis : les montants
  // stockés sont restés en FCFA.
  const legacy: BusinessOrder = {
    id: 'o3',
    ...
    transport_mode: 'sea',
    items_total: 496_000,
    transport_cost: 795_600,
    discount: 0,
    commission: 0,
    lines: [{ product_id: 'p9', quantity: 1, unit_price_cny: 5450, margin_percent: 45 }],
  };
  it('retient la devise qui colle aux lignes, pas celle du listing', () => {
    expect(effectiveCurrency(legacy)).toBe('XAF');
    expect(effectiveCurrency({ ...legacy, items_total: 708 })).toBe('EUR');
  });
  it('la marge reste juste : 45 % de marque → 31 % du CA, et non 99 %', () => {
    const m = orderMargin(legacy);
    expect(m.revenue).toBeCloseTo(5450 * 91, 0);
    expect(m.cost).toBeCloseTo((5450 / 1.45) * 91, 0);
    expect(m.marginRate).toBeCloseTo(1 - 1 / 1.45, 3);
  });
});
