...
export default function AdminWhatsappPage() {
  ...
  const [annMode, setAnnMode] = useState<'message' | 'poll'>('message');
  ...
  const sendPoll = async () => {
    ...
    if (!pollTitle.trim() || options.length < 2) {
      setPollStatus('❌ Titre + au moins 2 options requis');
      return;
    }
    setPollSending(true);
    setPollStatus(null);
    try {
      const res = await fetch('/api/whapi/poll', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title: pollTitle, options, multiple: pollMultiple }),
      });
      const data = await res.json();
      if (!res.ok) {
        setPollStatus(`❌ ${data.error || 'Échec de l’envoi'}`);
        return;
      }
      setPollStatus('✅ Sondage envoyé dans le groupe');
      setPollTitle('');
      setPollOptions(['', '']);
      setPollMultiple(false);
    } catch {
      setPollStatus('❌ Erreur réseau');
    } finally {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      {/* Annonce libre */}
      <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-800">
        {/* Bascule Message / Sondage */}
        <div className="mb-4 inline-flex rounded-xl bg-slate-100 p-1 dark:bg-slate-700/50">
          <button
            type="button"
            onClick={() => setAnnMode('message')}
            className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold transition-colors ${
              annMode === 'message' ? 'bg-white text-slate-900 shadow dark:bg-slate-800 dark:text-white' : 'text-slate-500'
            }`}
          >
          ...
          <button
            type="button"
            onClick={() => setAnnMode('poll')}
            className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold transition-colors ${
              annMode === 'poll' ? 'bg-white text-slate-900 shadow dark:bg-slate-800 dark:text-white' : 'text-slate-500'
            }`}
          >
        ...
        {annMode === 'message' ? (
          ...
          <>
            <p className="mb-2 text-xs text-slate-400">
              Sondage natif WhatsApp (interaction fiable). 2 à 12 options.
            </p>
            <input
              type="text"
              value={pollTitle}
              ...
              className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 focus:outline-none dark:border-slate-600 dark:bg-slate-700 dark:text-white"
            />
              ...
              {pollOptions.map((opt, i) => (
                <div key={i} className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-700 focus:outline-none dark:border-slate-600 dark:bg-slate-700 dark:text-white"
                  />
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3">
              <motion.button
                type="button"
                onClick={sendPoll}
                disabled={pollSending || !pollTitle.trim() || pollOptions.filter((o) => o.trim()).length < 2}
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="flex items-center gap-2 rounded-xl bg-[#25D366] px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-[#25D366]/25 disabled:opacity-60"
              >
                {pollSending ? <Loader2 className="h-4 w-4 animate-spin" /> : <BarChart3 className="h-4 w-4" />}
                Envoyer le sondage
              </motion.button>
              {pollStatus && (
                <span className="text-xs font-medium text-slate-600 dark:text-slate-300">{pollStatus}</span>
              )}
            </div>
          </>
        )}
