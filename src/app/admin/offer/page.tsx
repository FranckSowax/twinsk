    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {offers.length} offre(s) au total · partagez le lien public dans vos groupes
            et réseaux sociaux pour générer des ventes.
          </p>
        </div>
        <motion.button
          type="button"
          ...
          className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-emerald-500 to-green-500 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-emerald-500/25"
        >
      ...
      {createOpen && (
        <motion.div
          ...
          className="space-y-3 rounded-2xl border border-emerald-200 bg-emerald-50/40 p-5 dark:border-emerald-800 dark:bg-emerald-900/10"
        >
          ...
          <input
            type="text"
            placeholder="Titre de l'offre (ex: Pack démarrage restaurant)"
            value={title}
      ...
      {!offers.length ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center dark:border-slate-700 dark:bg-slate-800">
          ...
          <p className="mt-4 text-slate-500">Aucune offre pour le moment</p>
        </div>
      ) : (
