...
import { AFFILIATE_PAYOUT_COLUMN, affiliatePayoutNumber } from '@/lib/payments/methods';
...
async function loadLink(id: string) {
  const { data } = await supabaseAdmin
    ...
    .select('id, offer_id, commission_percent, hidden_product_ids, item_order, active, affiliates(*)')
    .eq('id', id)
    .single();
  if (!data) return null;
...
export async function GET(
  ...
  return NextResponse.json({
    ...
    // Clé historique « airtel_number » conservée : c'est le numéro d'encaissement (D5).
    airtel_number: affiliatePayoutNumber(link.affiliate),
...
export async function PATCH(
  ...
  if (typeof body.airtel_number === 'string') profile[AFFILIATE_PAYOUT_COLUMN] = body.airtel_number.trim() || null;
