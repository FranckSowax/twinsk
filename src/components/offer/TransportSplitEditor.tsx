...
export default function TransportSplitEditor({ lines, mixed, currency, applied, busy, onApply, compact = false }: Props) {
  ...
  const input = `${compact ? 'w-14' : 'w-16'} rounded-lg border border-slate-200 bg-white px-2 py-1 text-center text-sm tabular-nums dark:border-slate-600 dark:bg-slate-800 dark:text-white`;

  return (
    <div className="space-y-3 rounded-2xl border border-violet-200 bg-violet-50/40 p-4 dark:border-violet-800 dark:bg-violet-900/10">
      <p className="text-sm font-semibold text-slate-900 dark:text-white">Répartir chaque produit entre l’avion et le bateau</p>
          ...
          return (
            <li key={l.id} className="flex flex-wrap items-center gap-3 py-2">
              ...
              <span className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-300">
                ...
                <span className="w-8 text-center font-semibold tabular-nums">{l.quantity - air}</span>
                bateau
              </span>
              <div className="flex gap-1">
                <button type="button" onClick={() => setAir(l.id, l.quantity, l.quantity)} className="rounded-md border border-slate-200 px-1.5 py-0.5 text-[10px] text-slate-600 hover:bg-white dark:border-slate-600 dark:text-slate-300">tout avion</button>
                <button type="button" onClick={() => setAir(l.id, l.quantity, 0)} className="rounded-md border border-slate-200 px-1.5 py-0.5 text-[10px] text-slate-600 hover:bg-white dark:border-slate-600 dark:text-slate-300">tout bateau</button>
              </div>
            </li>
          );
        })}
      ...
      {applied && mixed && (
        <div className="grid gap-2 rounded-xl bg-white p-3 text-sm dark:bg-slate-800 sm:grid-cols-2">
          ...
          <p className="font-semibold text-slate-900 dark:text-white sm:col-span-2">
